...
function getAllManagers($data){
    $sql = "SELECT * FROM Personel WHERE User_Type='Manager'";
...
function addNewManager($inputData) {
    ...
    $result = mysqli_query($db, $sql);
     if($result){
         ...
         echo "Failed to add new Manager. Please contact the Administrator";
         http_response_code(422);
     }
}

function deleteManager($inputData) {
    $ID = $inputData[1] -> ID;
    ...
    if($result){
        ...
        echo "Failed to remove the Manager. Please contact the Administrator";
        http_response_code(422);
    } 
}
...
function alterRecord($inputData) {
    ...
    if($result){
        ...
        echo "Failed to update the Manager. Please contact the Administrator";
        http_response_code(422);
    } 
}
?>